...
# Example
    ...
    -43 press aim yaw -1.24         # ends the alt glitch and starts aiming at the right spot to exit spawn
    
    let's jump on the spawn
    0 press jump
...
- `aim` is a keyword starting expectation for two elements:
    ...
    - Only one of these, leaving the other as it was in the previous frame, eg. `aim pitch 0.856` or `aim yaw 1.114`.

- `fl` asks the engine to start holding freelook, and `nofl` asks for releasing it. As usual, holding won't work in some nonstandard gravity cases.
